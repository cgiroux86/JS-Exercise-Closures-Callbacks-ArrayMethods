...
*/
function processLastItem(strList, cb) {
...
function processSum(numList, cb) {
 return cb(numList.reduce((a,b) => a + b,0))
}
...
function processDuplicateFree(list,cb) {
  return Array.from(cb(new Set(list)))
}
...
function getFullNames(arr) {
  let newArr = [];
  arr.forEach(elem => newArr.push(`${elem.last_name}, ${elem.first_name}`))
  return newArr;
}
...
function firstNamesAllCaps(arr) {
  return arr.map(elem => elem.first_name.toUpperCase())
}
...
function getRunnersByTShirtSize(arr, size) {
  return arr.filter(elem => elem.shirt_size == size)
}
...
function tallyUpDonations(arr) {
  return arr.map(elem => elem.donation).reduce((a,b) => a + b,0)
